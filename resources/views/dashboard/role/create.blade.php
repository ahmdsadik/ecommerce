...
@section('title')
    {{ __('dashboard/role/create.title') }}
@endsection
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <h4 class="fw-bold py-3 mb-4">
            <a class="text-muted fw-light"
               href="{{ route('dashboard.roles.index') }}">{{ __('dashboard/role/create.breadcrumb_1') }}</a>
            /
            {{ __('dashboard/role/create.breadcrumb_2') }}
        </h4>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card mb-4">
                    <h5 class="card-header">{{ __('dashboard/role/create.card_header') }}</h5>
                    ...
                    <div class="card-body">
                        <form action="{{ route('dashboard.roles.store') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    @foreach(LaravelLocalization::getLocalesOrder() as $key =>  $locale)
                                        <div
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <label for="name"
                                                       class="form-label">{{ __('dashboard/role/create.name',['lang' => __('lang_key.with_'.$key)]) }}</label>
                                                <input class="form-control" type="text" id="display_name" name="{{$key}}[display_name]"
                                                       placeholder="{{ __('dashboard/role/create.name_placeholder',['lang' => __('lang_key.with_'.$key)]) }}"
                                                       value="{{ old($key.'.display_name') }}"/>
                            ...
                            <div class="card-footer mt-3">
                                <button class="btn btn-primary w-100" title="{{ __('dashboard/role/create.submit_btn') }}">
                                    {{ __('dashboard/role/create.submit_btn') }}
                                </button>
                            </div>
                        </form>
                    </div>
